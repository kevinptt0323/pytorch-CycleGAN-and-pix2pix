...
class TrapPix2PixModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        else:  # during test time, only load G, T
            self.model_names = ['G', 'T']
        # define networks (both generator, discriminator, and trapper)
        self.netG = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.norm,
                                      not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)

        self.netT = networks.define_T(opt.input_nc, (opt.crop_size, opt.crop_size), opt.ntf, opt.stf, opt.norm,
    ...
    def set_input(self, input):
        ...
        self.real_A = self.netT(self.real_B)
    ...
    def backward_T(self):
        self.loss_T = -1 * self.criterionL1(self.fake_B, self.real_B) * self.opt.lambda_L1
